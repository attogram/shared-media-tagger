...
class smt_admin_database extends smt {
	...
	function save_images_to_database($images='', $category='') {
		...
		while( list(,$image) = each($images) ) {
			//$this->notice('::save_images_to_database: IMAGE=' . print_r($image,1) );
			...
			//$this->notice('::save_images_to_database: IMAGE: <small>' . print_r($i,1) .'</small>' );
			...
			// connect category
			//$this->notice(' + ::save_images_to_database: CATEGORY: '. $category . ' id=' . $category_id);
			$r = $this->query_as_bool(
			...
			if( !$r ) { 
				...
				/*$this->notice(' + :: SAVED category2media: (insertid:' . $this->last_insert_id
					. ')(pageid:'. $i[':pageid']
...
'default_site' => "INSERT INTO site (id, name, about) VALUES (1, 'Reviews of Free Cultural Works', 'This is a demonstration of the Shared Media Tagger software.')",
...
class smt_commons_API extends smt_admin_database {
	...
	function call_commons($url, $key='') {
		$this->debug("::call_commons: url=$url key=$key)");
    ...
    function get_media_from_category($category='') {
		...
		foreach( $chunks as $chunk ) {
			...
			$r = $this->save_images_to_database( $this->get_api_imageinfo($chunk), $category );	
		}
	...
	function get_api_categorymembers( $category ) {
		...
		. '&cmlimit=500'
		. '&cmtitle=' . urlencode($category);
		$call = $this->call_commons($url, $key='categorymembers');
	...
	function get_random_images_from_api($limit=2) {
		$this->debug("::get_random_images_from_api($limit)");
		$call = $this->commons_api_url . '?action=query&format=json'
		. '&generator=random'
		. '&grnlimit=' . $limit
		. '&grnnamespace=6'
...
class smt_admin extends smt_commons_API {
	...
	function __construct( $title='' ) {
		...
		ini_set('user_agent','Pic Tagger Test');
		$this->api_count = 0;
