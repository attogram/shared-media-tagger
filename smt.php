...
<?php print $space; ?>
<span class="nobr menu_highlight"><b><a href="<?php print $this->url('home'); ?>">Review a file</a></b></span>
<?php print $space; ?>
...
<span class="nobr"><b><a href="<?php print $this->url('reviews'); ?>"><?php 
	print $this->get_total_review_count(); ?> Reviews</a></b> on <?php 
	print $this->get_total_files_reviewed_count(); ?> of <?php 
	print $this->get_image_count(); ?> files</span>
<?php print $space; ?>
...
class smt EXTENDS smt_site_utils {
	...
	function display_thumbnail_box( $media='' ) {
		...
		. '<br /><div class="thumbnail_reviews">' . $this->get_reviews( $media['pageid'] ) . '</div>'
		. '</div>';
		return $r;
	}
	...
	function display_categories( $media_id ) {
		...
		. '<span style="font-size:80%;">Categories:</span>';
		if( !$cats ) { $r .= '<br /><em>Uncategorized</em>'; }
		foreach($cats as $cat ) {
			$r .= '<br />'
			. '<a href="' . $this->url('category') 
			. '?c=' . $this->category_urlencode( $this->strip_prefix($cat) ) . '">' 
			. $this->strip_prefix($cat) . '</a>';
		}
		return $r . '</div>';
	}
